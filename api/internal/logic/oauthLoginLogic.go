...
func (l *OauthLoginLogic) OauthLogin(req *types.OauthLoginReq) (resp *types.BaseResp, err error) {
	...
	// 2.6 Status check: 被禁用/暂停的账号不允许登录
	if user.GetStatus() != "active" {
		return &types.BaseResp{Code: errno.AccountDisabled.Code, Msg: errno.AccountDisabled.Msg}, nil
	}
	...
	// 登录不递增 token_version（允许多会话共存，避免重复登录/多标签页互相踢出）。
	// 当前版本号读取自 Redis；权限变更（分配角色/API）或密码变更时会递增以踢掉旧会话。
	// 注意：必须用 Setex 确保 key 存在（首次登录时），否则 jwtauth 校验版本时 Get 返回空 → 401。
	tvKey := fmt.Sprintf("token_version:%d", user.GetId())
	tvStr, _ := l.svcCtx.Redis.Get(tvKey)
	tv, _ := strconv.ParseInt(tvStr, 10, 64)
	l.svcCtx.Redis.Setex(tvKey, strconv.FormatInt(tv, 10), int(l.svcCtx.Config.JwtExpire)*2)
	claims := &jwt.Claims{
		UserId:       user.GetId(),
		TenantId:     tenantId,
